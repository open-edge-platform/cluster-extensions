...
test_ephemeral_container_with_privileged_false {
...
test_exempt_image_within_namespace_container_with_privileged_true {
	input := input_obj(review_pod1(pod_spec1("containers", "sample-image:v1.0.0", true), "exemptNamespace"))
	results := violation with input as input

	count(results) == 0
}

test_exempt_image_within_namespace_container_with_privileged_false {
	input := input_obj(review_pod1(pod_spec1("containers", "sample-image:v1.0.0", false), "exemptNamespace"))
	results := violation with input as input

	count(results) == 0
}

test_exempt_image_within_namespace_container_without_security_context {
	input := input_obj(review_pod1(pod_spec2("containers", "sample-image:v1.0.0"), "exemptNamespace"))
	results := violation with input as input
