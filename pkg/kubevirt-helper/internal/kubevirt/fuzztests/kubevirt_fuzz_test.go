...
func isUninterestingErr(err error) bool {
	var jsonMarshalErr *json.UnmarshalTypeError
	var jsonSyntaxErr *json.SyntaxError
	if errors.As(err, &jsonMarshalErr) {
		return true
...
// FuzzGetVMsFromAdmissionRequest fuzzes Handler.GetVMsFromAdmissionRequest with random AdmissionRequests.
func FuzzGetVMsFromAdmissionRequest(f *testing.F) {
	ctx := context.Background()
	handler := kubevirt.NewKubevirtHandler()
	require.NotNil(f, handler)
	f.Add(string(testVMNew), string(testVMOld))
	f.Fuzz(func(t *testing.T, newRaw string, oldRaw string) {
		...
		vmOld, vmNew, err := handler.GetVMsFromAdmissionRequest(ctx, req)
		if isUninterestingErr(err) {
			return
		}
...
// FuzzCompareVMSpec fuzzes Handler.CompareVMSpec with random VirtualMachine objects.
func FuzzCompareVMSpec(f *testing.F) {
	ctx := context.Background()
	handler := kubevirt.NewKubevirtHandler()
	f.Add(string(testVMNew), string(testVMOld))
	f.Fuzz(func(t *testing.T, newRaw string, oldRaw string) {
		...
		_, _, err = handler.CompareVMSpec(ctx, vmNew, vmOld)
		assert.NoError(t, err, "%#v", err)
	})
}
